...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Até mais @${num.split('@')[0]}\nEspero que n�o volte nunca mais`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*Espera ai po to fazendo, acha que � f�cil? Fa�a voc� ent�o*',
				success: '✓Sucesso✓',
	...
	case 'kbbi':
	      ...
	      tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'tm':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*ã€Œ TRANSMISS�O ã€*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
				...
				case 'chord':
					...
					break
				case 'lirik':
					if (args.length < 1) return reply('Qual � o nome da m�sica mano?')
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `O dono deste belo grupo �  : @${from.split("-")[0]}`,
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (Number(args[0]) === 1) {
						...
						reply('✔️ SUCESSO boas vindas ativo com sucesso😳')
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'shota':
				    try{
						res = await fetchJson(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSflz1NeDfufhlwkxbknD1jYFcTs-Jjq1bzIA&usqp=CAU`)
						buffer = await getBuffer(res.result)
                                                if (!isUser) return reply(mess.only.daftarB)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ta ai a xota'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('⚠️ *ERRO* ⚠️')
					}
            ...
            break
				...
				case 'clone':
			 	...
			 	case 'buscanime':
					...
					if (args.length < 1) return reply('digite 1 para ativar ')
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('O anti-link j� est� ativo')
						antilink.push(from)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('Modo anti-link ativado com sucesso, quem mandar link de outros grupos � ban')
						...
						if (!isantilink) return reply('O modo  anti-link foi desabilitado ')
						var ini = anti.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./src/antilink.json', JSON.stringify(antilink))
						reply('Desativar  anti-link com sucesso neste grupo ')
					} else {
						reply('1 para ativar, 0 para desativar ')
					}
					break
